...
type PhysicalDeviceCaps struct {
	Properties *objects.PhysicalDeviceProperties
	Features *VKng.PhysicalDeviceFeatures

	GraphicsQueueFamily *int
	PresentQueueFamily *int

	Extensions map[string]*VKng.ExtensionProperties
...
func (c *PhysicalDeviceCaps) Suitability() int {
	...
	if c.Properties.Type == objects.DiscreteGPU {
		score += 1000
	}
...
func CreatePhysicalDeviceCaps(allocator cgoalloc.Allocator, device *objects.PhysicalDevice, surface *ext_surface.Surface) (*PhysicalDeviceCaps, error) {
	var err error
	...
	for queueFamilyIdx, queueFamily := range queueFamilies {
		if caps.GraphicsQueueFamily == nil && (queueFamily.Flags & VKng.Graphics) != 0 {
